...
- **Remote Tofu state**: state lives in a `tofu-state` bucket on the same MinIO instance as Velero backups (`tofu/backup.tf`), reached via an Ingress at `minio.k8s.thepugh.family` (`tofu/state-backend.tf`) with native S3 conditional-write locking. Bucket/key/endpoint are hardcoded literals in `tofu/terraform.tf`'s `backend "s3"` block — backend config can't reference variables. Credentials come from a gitignored `tofu/backend.hcl` (see `tofu/backend.hcl.example`), retrieved from the live cluster's `minio-credentials` Secret rather than `tofu/secrets.enc.yaml`, since they're Tofu-generated and only exist in state. A from-scratch rebuild needs a similar two-phase dance to Keycloak's (local state first, since Tofu creates MinIO itself, then `tofu init -migrate-state` once it exists) — see `docs/src/bootstrap-environment/09-remote-state.md`.

## Toolchain

- **OpenTofu** (not Terraform) — `tofu` CLI, config lives in `tofu/`. State is remote (see Kubernetes cluster section above) — a fresh machine needs `tofu init -backend-config=backend.hcl` (see `tofu/backend.hcl.example`) before anything else, not a bare `tofu init`.
...
- **No actual mechanism for running two environments from this config yet** — environment-specific values are variablized (see "What this repo is" above), but there's still one flat root module. Nothing today lets `tofu apply` stand up a second, independent cluster (e.g. prod) from the same config without clobbering dev's state — and now that the state backend itself (`tofu/terraform.tf`'s `backend "s3"` block) is a set of hardcoded literals too (bucket/key/endpoint can't be variables), a second environment needs those hand-edited as well as everything else. Tracked as issue #26 (Terragrunt), which would replace the hand-edited literals with a generated per-environment config.
...
- Moved Tofu state to a remote S3 backend on the existing MinIO instance (closed issue #12), reusing it rather than standing up a separate object store — a new `tofu-state` bucket alongside Velero's `velero` bucket, exposed to the local `tofu` client via a new Ingress since MinIO previously had no reachable endpoint outside the cluster. Chose OpenTofu's native S3 conditional-write locking over a DynamoDB-style lock table (MinIO doesn't speak DynamoDB); verified with a real test, not just the config flag — two concurrent `tofu plan` runs, the second rejected with an S3 `412 PreconditionFailed`. Surfaced a real chicken-and-egg problem in the process (MinIO is created *by* this Tofu config, so a from-scratch rebuild needs local state first, then a migrate-state step) and a structural one (backend blocks can't reference variables, so bucket/key/endpoint are hardcoded literals) — the latter is exactly what prompted filing issue #26 (Terragrunt) as future work.
