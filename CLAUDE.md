...
- **Backup**: Velero + MinIO (`tofu/backup.tf`), also Tofu/Helm-managed rather than GitOps — MinIO is an S3-compatible gateway in front of the NFS share (its own data volume is just another PVC on the `nfs_storage`-backed StorageClass), and Velero backs up Kubernetes object definitions to it on a daily schedule (`var.backup`). File-system/volume-snapshot backup is deliberately disabled since everything today is already NFS-backed and separately protected by the NAS's own offsite backup. Detailed docs, including the restore procedure: `docs/src/bootstrap-environment/07-backup-restore.md`.
- **SSO**: Keycloak + a hand-rolled Postgres `StatefulSet` (`tofu/sso.tf`), reachable at `keycloak.k8s.thepugh.family`. The `homelab` realm and its OIDC clients are managed declaratively via the `keycloak` Terraform provider — same pattern as `unifi_network.this` — authenticating with the same Tofu-generated bootstrap admin credentials Keycloak itself is given. A from-scratch cluster rebuild needs one `tofu apply -target=time_sleep.wait_for_keycloak` before the normal `tofu apply` completes (the provider can't configure until that password exists in state) — see `docs/src/bootstrap-environment/08-sso.md`. An oauth2-proxy + `whoami` forward-auth demo at `sso-demo.k8s.thepugh.family` is a permanent reference deployment, not a throwaway — it's the copy-paste template for real apps that need forward-auth.
- **Remote Tofu state**: a plain `local` backend (`tofu/terraform.tf`) pointed at `tofu/state/dev/terraform.tfstate`, a gitignored directory meant to be an SMB mount of a NAS share, set up out-of-band (not Tofu-managed) — chosen specifically so state storage never depends on infrastructure this config itself creates/destroys (no "destroying your own state backend" risk on a full teardown) and without introducing a new external dependency (cloud object storage). `path` is a hardcoded literal — backend config can't reference variables. See `docs/src/bootstrap-environment/09-remote-state.md` for the SMB mount setup and the alternatives considered (a MinIO-on-cluster S3 backend was tried first and reverted).

## Toolchain

- **OpenTofu** (not Terraform) — `tofu` CLI, config lives in `tofu/`. State lives on an SMB-mounted NAS share (see Kubernetes cluster section above) — a fresh machine needs that share mounted at `tofu/state/` before running `tofu init`, or `tofu` will happily create a fresh empty local state file instead of finding the real one.
...
- **No actual mechanism for running two environments from this config yet** — environment-specific values are variablized (see "What this repo is" above), but there's still one flat root module. Nothing today lets `tofu apply` stand up a second, independent cluster (e.g. prod) from the same config without clobbering dev's state — and the state backend's own `path` (`tofu/terraform.tf`'s `backend "local"` block) is a hardcoded literal too (backend blocks can't reference variables), so a second environment needs that hand-edited as well as everything else. Tracked as issue #26 (Terragrunt), which would replace the hand-edited literal with a generated per-environment config.
