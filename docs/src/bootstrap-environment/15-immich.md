...
1. **The claim has to be a bare string, not an array — because of the specific Immich version deployed.** `oauth.roleClaim` (`tofu/modules/keycloak-realm/main.tf`'s `keycloak_openid_user_client_role_protocol_mapper.immich_role`) has to resolve to the literal string `"admin"`, not an array containing it. This is genuinely version-specific: `main` branch Immich checks `roles.includes('admin')` (an array is fine), but the deployed `v3.0.0` release checks `typeof value === 'string'` (`getClaim` in `auth.service.ts`) — an array value silently fails that check and falls back to the `"user"` default, no error, no log. Found live as a real "logged in via Keycloak but landed as a normal user" bug. This is why the mapper is a **client role** scoped to Immich's own client (`client_id_for_role_mappings`) with `multivalued = false`, not the more obvious realm-role approach: a realm-role mapper's claim is unavoidably an array, since every user also carries Keycloak's own `default-roles-homelab`/`offline_access`/`uma_authorization` realm roles alongside "admin" — there's no way to filter a realm-role mapper down to just one role. A client-role mapper scoped to one client sidesteps this entirely, since Immich's client has no other client roles to compete with.
2. **`client_id_for_role_mappings` wants the client's `client_id` string, not its Keycloak-internal UUID** — despite the field name suggesting otherwise. Passing `keycloak_openid_client.immich.id` (the UUID) produces a mapper that matches nothing, silently omitting the claim from the token entirely (verified via Keycloak's own `evaluate-scopes/generate-example-id-token` admin API, which is a much faster way to check what a mapper actually produces than a real login round-trip). The correct value is `keycloak_openid_client.immich.client_id` (the string `"immich"`).

Also confirmed live: in `v3.0.0`, `roleClaim` is evaluated **only at account creation**, not on every login (a newer, not-yet-released Immich version does re-evaluate on every login — revisit this note once that ships). A role change here does not retroactively fix an already-created Immich user; delete the user (`DELETE FROM "user" WHERE email = ...` — Immich's own foreign keys all cascade, confirmed via `pg_constraint` before doing this live) and let their next login recreate it, or use `immich-admin grant-admin`/`revoke-admin` (a CLI available via `kubectl exec` into the server pod) to patch an existing account directly instead.
