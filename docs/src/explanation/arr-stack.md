...
**Not yet exercised**: an actual grab through each app's own UI (searching for a real show/movie in Sonarr/Radarr and confirming a real download completes and gets hardlinked into `/media/Movies` or `/media/TV Shows`) — everything up to that point is wired and confirmed working, but this last step needs a real interactive UI session. Also unresolved: whether Kubernetes NetworkPolicy meaningfully constrains Gluetun's post-tunnel `tun0` traffic or only its pre-tunnel `eth0` traffic — Gluetun's own internal kill switch is the real leak-prevention mechanism regardless of how that turns out, so this was left as a documented open question rather than chased further.
