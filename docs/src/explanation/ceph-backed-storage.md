...
**CephX credentials** (Ceph's own internal auth, separate from Proxmox's PVE user management) can't be Tofu-managed — see the [Deploy From Scratch](../guides/deploy-from-scratch.md) guide's prerequisites for the required manual bootstrap step, and [Stand Up a New Environment](../guides/stand-up-a-new-environment.md) for repeating it on a new environment. `ceph_rbd_client_key` in `secrets.enc.yaml` is keyed **per environment** (`{dev: ..., prod: ...}`), not a single flat value — dev and prod each get their own CephX auth entity scoped to their own pool, and a shared single key silently breaks whichever environment doesn't currently own the stored value (found live the first time dev and prod coexisted: `ceph-csi`'s connection failed `rados: ret=-13, Permission denied` rather than the Tofu apply itself hard-failing). If `ceph-csi`'s provisioner already attempted and failed a connection before the correct key was in place, it can cache that failure — `kubectl rollout restart deployment -n ceph-csi-rbd ceph-csi-rbd-provisioner` forces it to reconnect with the current Secret.
