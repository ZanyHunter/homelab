...
### Two real gotchas found live deploying Homepage

Both surfaced as the container crashing well past readiness — a server-side probe failure and a 500 on the OIDC sign-in path, not a config-looks-wrong problem:
...
2. **The image auto-populates *every* skeleton config file it doesn't find, and can't write them as non-root.** Only the 4 files Homepage's own config actually needs (`services.yaml`, `widgets.yaml`, `settings.yaml`, `bookmarks.yaml`) were originally mounted; at startup the entrypoint also tries to copy `kubernetes.yaml`/`docker.yaml`/`custom.css`/`custom.js` from its own image into `/app/config` for any of those it doesn't find — and crashed with `EACCES` the moment a real request needed one, since `/app/config`'s directory isn't writable under `runAsNonRoot`. The same failure mode also broke logging: Homepage writes its own log file under `/app/config/logs`, and without a separate writable mount there, trying to log the *original* error crashed the request with a second, more confusing `EACCES` instead — a real OIDC auth attempt surfaced as an opaque 500 with no useful log line until this was fixed. Fixed by mounting all 8 skeleton files (4 real, 4 empty) via `subPath` plus a dedicated `emptyDir` at `/app/config/logs`, matching gethomepage.dev's own Kubernetes example exactly rather than the trimmed-down 4-file mount that looked sufficient from the docs alone. Verified live end-to-end: the sign-in flow now correctly redirects to Keycloak's real authorization endpoint with the right `client_id`/`redirect_uri`/PKCE challenge.
