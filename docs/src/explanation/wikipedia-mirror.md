...
~162GB at rest. Project Gutenberg and Stack Exchange were deliberately left out of this bundle entirely (not just out of dev's smaller subset below): both are structured very differently from the Wikimedia-family ZIMs above (Gutenberg splits into dozens of per-LCC-subject files, Stack Exchange is one file per site — `serverfault.com`, `superuser.com`, etc.) and would need their own scope decision rather than folding in blindly.

This full six-series table is what **prod** carries. **Dev deliberately carries less** — see the per-environment section below.

## No database, no Ceph, no backup

The whole app is a single **NFS-backed** PVC (`wikipedia-data`) holding the ZIM files plus `kiwix-serve`'s `library.xml` — no Ceph-backed component at all, the first app in this repo's `apps/` tree with no database. Its size is per-environment (see below): prod's 320Gi isn't just the ~162GB steady-state total, since the update mechanism downloads a series' new release fully before deleting the old one, so during a Wikipedia Maxi update specifically — the largest series — old and new briefly coexist, pushing peak usage to ~286GB; 320Gi leaves headroom above that peak for future release growth too.
...
The PVC's size is environment-specific too, via the same `env-values.yaml`/`replacements:` mechanism already used for hostnames and StorageClass names (a new `pvc-size` key, substituted into `data-pvc.yaml`'s `__DATA_PVC_SIZE__` token): 320Gi for prod's full bundle, 5Gi for dev's two-series subset. Since Kubernetes PVCs can only grow, never shrink, moving dev from its original 320Gi (provisioned before this split existed) down to 5Gi needed the PVC deleted and recreated, not just reapplied — safe here since `nfs-dev`'s StorageClass has `reclaimPolicy: Delete` (the backing NFS subdirectory is actually removed, not orphaned) and this app's data is a re-fetchable cache to begin with.
...
Verified live on dev, including the final per-environment-scoped shape (not just an earlier intermediate version): the Deployment reaches `1/1 Running` with an empty `library.xml` written by its own init container (confirmed via container logs matching a local Docker reproduction exactly), the Ingress/TLS certificate come up cleanly (`kubectl get certificate` `READY: True`, a real `HTTP 200` from the hostname), and — after the PVC was deleted and recreated at dev's final 5Gi size — a manually-triggered run of the scoped-down `CronJob` (just the `sync-wikiquote`/`reindex-wikiquote`/`sync-wikivoyage`/`reindex-wikivoyage` steps) reached `Completed` cleanly, with both series downloaded (passing size checks against their own `.meta4` metadata), both entries present in `library.xml`, and both titles confirmed live via `kiwix-serve`'s own `/catalog/v2/entries` endpoint. `df -h` inside the pod confirmed real usage (~4.5Gi of the 5Gi PVC) matching the two-series footprint, not the earlier six-series one. Dev's `Application` in ArgoCD has not yet picked this up via a real sync (verified via direct `kubectl apply`, this repo's established pre-PR pattern) — that happens automatically once this change merges to `development`. Prod has never had this app applied at all; whoever applies it there gets the full six-series bundle with no further changes needed, since prod's overlay carries no scope-reducing patch.
