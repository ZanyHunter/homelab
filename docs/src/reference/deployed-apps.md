...
| [qBittorrent](../explanation/arr-stack.md) | `qbittorrent.dev.thepugh.family` | SQLite (bundled), `ceph-rbd-dev` (5Gi) | Downloads: static NFS PV (dev: isolated test path; prod: `jellyfin-media/Downloads`) | Keycloak OIDC via oauth2-proxy (forward-auth); all traffic routed through Gluetun/Proton VPN (Switzerland) — the one `privileged`-PSA app namespace in this repo. **Tunnel connects, but actual torrenting doesn't work on Proton's free plan** — it blocks P2P traffic outright; see the explanation page |
