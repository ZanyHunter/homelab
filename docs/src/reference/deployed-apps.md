...
| [Pinchflat](../explanation/self-hosted-apps.md) | `youtube.dev.thepugh.family` | SQLite (bundled), `ceph-rbd-dev` (5Gi) | Videos: static NFS PV, `truenas.thepugh.family:/mnt/Main/jellyfin-media/YouTube` (shared with prod and with Jellyfin's own media export, read-write) | Keycloak OIDC via oauth2-proxy (forward-auth) |
