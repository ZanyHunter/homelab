...
**A second writer as of Pinchflat's storage redesign**: `/mnt/Main/jellyfin-media/YouTube`, a subdirectory inside this same export, holds Pinchflat's downloaded videos (`apps/youtube-archiver/base/downloads-pvc.yaml`, its own static PV, read-write). This directory needs to be created by hand and `chown`'d to `1000:1000` before that PV/PVC is applied — both Pinchflat and Jellyfin already run as that exact uid/gid, so no broader world-writable/world-readable treatment is needed the way the rest of this export's content required. No export-level (TrueNAS share) change was needed for this — the export was already a plain read-write NFS share reachable from both VLANs; Jellyfin's own read-only behavior has only ever been enforced client-side via its PV's `mountOptions: [ro]`. See [Jellyfin](../explanation/jellyfin.md#sharing-this-export-with-pinchflats-downloaded-videos) and [Self-Hosted Apps](../explanation/self-hosted-apps.md) for the full reasoning, including why this is shared across dev/prod rather than split per-environment like every other app's storage.
